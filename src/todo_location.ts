...
export class ToDoLocation {
     ...
     * @param path
     */
    public static fromUnknownPosition(path: string): ToDoLocation {
        return new ToDoLocation(path, 0, 0, 0, null);
    }
    ...
    fromRenamedFile(newPath: string) {
        return new ToDoLocation(newPath, this.lineNumber, this.sectionStart, this.sectionIndex, this.precedingHeader);
    }
